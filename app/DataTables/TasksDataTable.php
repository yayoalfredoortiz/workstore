...
class TasksDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return datatables()
            ...
            ->addColumn('timeLogged', function ($row) {
                ...
                if($row->timeLogged)
                {
                    $timeLog = intdiv($row->timeLogged->sum('total_minutes'), 60) . ' ' . __('app.hrs') . ' ';

                    if ($row->timeLogged->sum('total_minutes') % 60 > 0) {
                        $timeLog .= $row->timeLogged->sum('total_minutes') % 60 . ' ' . __('app.mins');
                    }
                }
            ...
            ->rawColumns(['board_column', 'action', 'project_name', 'clientName', 'due_date', 'users', 'heading', 'check', 'timeLogged'])
    ...
    public function query(Task $model)
    {
        ...
        $model = $model->leftJoin('projects', 'projects.id', '=', 'tasks.project_id')
            ...
            ->with('users', 'activeTimerAll', 'boardColumn', 'activeTimer', 'timeLogged')
            ->groupBy('tasks.id');
