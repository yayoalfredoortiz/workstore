...
class LeadNoteController extends AccountBaseController
{
    ...
    public function show($id)
    {
        ...
        abort_403(!($viewClientNotePermission == 'all'
            ...
            || ($viewClientNotePermission == 'owned' && $this->note->lead_id == user()->id)
            || ($viewClientNotePermission == 'both' && ($this->note->lead_id == user()->id || $this->note->added_by == user()->id))
            || (in_array('client', user_roles()) && ($this->note->type == 0 || $this->note->is_lead_show == 1))
            )
        );
    ...
    public function store(StoreLeadNote $request)
    {
        ...
        $note->is_lead_show = $request->is_lead_show ? $request->is_lead_show : '';
    ...
    public function update(StoreLeadNote $request, $id)
    {
        ...
        $note->details = $request->details;
        $note->type = $request->type;
        $note->is_lead_show = $request->is_lead_show ?: '';
    ...
    {
        $this->contact = LeadNote::findOrFail($id);
